...
Agents writing into `_CHANGELOG/` must follow these routing rules. Project-
...
ticket-source markdown stay outside `_CHANGELOG/`. Convert executable work into
...
```bash
python3 "$HOME/gitrepos/02_workskills/project-changelog/scripts/close_backlog.py" \
  <backlog_id> --resolved-by <change_id> --changelog-dir _CHANGELOG
```
...
```bash
python3 "$HOME/gitrepos/02_workskills/project-changelog/scripts/import_backlog.py" \
  --source path/to/backlog.md --changelog-dir _CHANGELOG
```
...
- Detailed run artefacts live outside `_CHANGELOG/`, usually in
...
- Put code, generated reports, or chat transcripts in `_CHANGELOG/`
